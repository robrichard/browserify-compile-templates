...
module.exports = function (html, options) {
    ...
    var writeOutput = new htmlparser.DomHandler(function (error, dom) {
        ...
        if(!compiledTemplates.length){
                ...
                output += '\n'
                if(exportedTemplateNames.length){
                    output += 'module.exports = {\n';
                    for (var i = 0, len = exportedTemplateNames.length; i < len; i++) {
                        if(i!=0){
                            output += ',\n';
                        }
                        var t = exportedTemplateNames[i];
                        output += t +':' + t;
                    }
                    output += '\n};';
                }
